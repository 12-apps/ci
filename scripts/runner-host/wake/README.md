...
- **Reclaimed hosts.** A job on a host that AWS reclaims fails with it. When a run completes as a failure, the function looks for failed jobs that lost their runner (GitHub closes those with the running step left unfinished; a job that fails on its own finishes every step it ran) and re-runs the run's failed jobs. This happens up to the third attempt. A job that failed on its own merits never triggers a re-run.
